...
if ($logged == 'No') {
    $response['message'] = 'Vous devez être connecté pour ajouter un favori.';
    echo json_encode($response);
    exit;
}

if (!isset($_POST['post_id'])) {
    $response['message'] = 'ID de post manquant.';
    echo json_encode($response);
    exit;
}
...
try {
    if ($has_favorited) {
        ...
        $response['message'] = 'Favori retiré.';
    } else {
        ...
        $response['message'] = 'Favori ajouté.';
    }
